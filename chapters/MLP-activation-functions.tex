...
{(1/(1+e^(-x)))*(1-(1/(1+e^(-x))))}
node[right,pos=1] {$\sigma(x)(1-\sigma(x))$};
\addplot[
	samples = 100, 
	color = orange,
	thick, smooth,
...
\end{axis}
\end{tikzpicture}
\caption{Sigmoid Function (red) and its derivative (blue).}
\end{figure}

\subsection{Softmax Function}
In a classification problem we would like $a_j$ to be a probability ($\Rightarrow \sum_i a_i = 1$). % TODO outputs posteriori probability, biggest value is very likely to get close to 1
...
Similar to Sigmoid function, but if the input has a mean of $0$ then so will the output. % TODO Costly computation?
\begin{equation}
